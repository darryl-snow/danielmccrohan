	...
	class JSON_API_Custom_Controller {
		...
		public function get_books() {
			...
			while($loop->have_posts()) : $loop->the_post();
				...
				$array['books'][$counter]['date'] = strtotime($array['books'][$counter]['publisheddate']);
				...
				foreach($custom as $k => $v) {
					$array['books'][$counter][$k] = array_shift($v);
				}
		...
		public function get_latest_book() {
			...
			$args = array('post_type'=>'book', 'posts_per_page'=>1,'oderby'=>'date', 'order'=>'DESC');
			$loop = new WP_Query($args);

			while($loop->have_posts()) : $loop->the_post();

				$array['book']['id'] = get_the_id();
				$array['book']['title'] = get_the_title();
				$array['book']['content'] = get_the_content();
				$array['book']['summary'] = get_the_excerpt();
				$array['book']['relatedcontent'] = get_post_meta(get_the_id(), 'relatedcontent', true);
				$array['book']['buylink'] = get_post_meta(get_the_id(), 'link', true);
				$array['book']['publisher'] = get_post_meta(get_the_id(), 'publisher', true);
				$array['book']['publisheddate'] = get_post_meta(get_the_id(), 'publisheddate', true);
				$array['book']['date'] = strtotime($array['book']['publisheddate']);
				$array['book']['buylinktitle'] = get_post_meta(get_the_id(), 'linktitle', true);
				$array['book']['comingsoon'] = get_post_meta(get_the_id(), 'comingsoon', true);
				$array['book']['hasvideo'] = get_post_meta(get_the_id(), 'hasvideo', true);
				$image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full');
				$image_url = $image_url[0];
				$array['book']['featuredimage'] = $image_url;
		...
		public function get_articles() {
			...
			while($loop->have_posts()) : $loop->the_post();
				...
				$array['articles'][$counter]['publisheddate'] = get_post_meta(get_the_id(), 'publisheddate', true);
				$array['articles'][$counter]['date'] = strtotime($array['articles'][$counter]['publisheddate']);
				$image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full');
				$image_url = $image_url[0];
				$array['articles'][$counter]['featuredimage'] = $image_url;
		...
		public function get_article() {
			...
			$array['article']['featuredimage'] = $image_url;
